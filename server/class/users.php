                    ...
                    if($is_disabled != 1)
                    {
                        if($is_locked != 1)
                        {
                            ...
                            if($work_day['code'] != "44")
                            {
                                ...
                                {
                                    $church_details = $this->getItemLabelArr('userdata',array('username'),array($result[0]['username']),array('password'));
    ...
    public function userlist($data)
    {
		...
		$columner = array(
            ...
            array( 'db' => 'username',   'dt' => 9, 'formatter'=>function($d,$row){
                ...
                {
                    return  $sack."<button onclick=\"trigUser('".$d."','".$row['user_disabled']."')\" class='btn btn-sm ".$locking_class."'>".$locking."</button><a class='btn btn-sm btn-warning'   onclick=\"getModal('setup/edit_user.php?op=edit&username=".$d."','modal_div')\"  href=\"javascript:void(0)\" data-toggle=\"modal\" data-target=\"#defaultModalPrimary\" >EDIT THIS USER</a>";
                }
